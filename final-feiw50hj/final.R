...
name <- "Chrisopher Liu"
github.name <- "cliu412"
email.address <- "cliu412@berkeley.edu"
...
# y[[2]] is 2, 4, 6, ..., 1000
# y[[3]] is 3, 6, 9, ..., 999
y <- list(seq(1,1000,by=1),seq(2,1000,by=2),seq(3,1000,by=3),seq(4,1000,by=4),seq(5,1000,by=5)
          ,seq(6,1000,by=6),seq(7,1000,by=7),seq(8,1000,by=8),seq(9,1000,by=9),seq(10,1000,by=10))
...
set.seed(42)
z <- sample(1:2,5000,replace=T,prob=c(0.3,0.7))==1
...
set.seed(344)
m <- matrix(data=rnorm(100,mean=8),nrow=10,ncol=10)
...
cw4 <- subset(ChickWeight,sample(1:nrow(ChickWeight),200,replace=F)==row.names(ChickWeight))
