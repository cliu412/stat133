...
subset.nosilver <- subset(SO2012Ctry,Silver==0)
subset.threesilver <- subset(SO2012Ctry,Silver>=3)
...
meanpopByGDPPP <- function(GDPPP.cutoff, GDPPP, pop){
  j<-data.frame(GDPPP,pop)
  j<-subset(j,GDPPP>GDPPP.cutoff)
 return (mean(j$pop))
}
...
plot(SO2012Ctry$Female+SO2012Ctry$Male,female.prop,pch=19,
