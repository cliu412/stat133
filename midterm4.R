...
hotelCal.split <- unlist(strsplit(hotelCal.split,"[[:blank:]]+"))
...
updateDate <- function(dates, old.yr) {
  indices <- grep(old.yr, dates)
  new.dates <- gsub(old.yr,2015,dates)
  return (new.dates[indices])
}
